...
import com.example.filedemo.payload.AuditColis;
...
public interface ColisRepository extends JpaRepository<Colis, Long> {
	...
	@Query(value = "SELECT c.etat ,c.anomalie_id, r.revtstmp FROM revinfo r , colis_aud c WHERE c.reference = :reference and c.rev = r.rev", nativeQuery = true)
	public List<AuditColis> getColisAud(@Param("reference") Long reference);
	...
	@Query(value = "SELECT * FROM colis c LEFT JOIN dispatch d on c.dispatch_id_dispatch"
			...
			+ " d.livreur_iduser= :idLivreur And d.id_dispatch = ("
			+ "	SELECT id_dispatch FROM dispatch d where d.livreur_iduser= :idLivreur ORDER BY date_creation DESC,id_dispatch DESC LIMIT 1)", nativeQuery = true)
	public List<Colis> findColisDispatchByIdLivreurForDbrf(@Param("idLivreur") Long idLivreur);
