...
public class PDFGenerator3 {
    ...
    public static ByteArrayInputStream runsheetPDFReport(Runsheet r, String barCodeColisDirectoryPath, 
        ...
        try {
            ...
            for(int j=0; j<colisEtats.size();j++) {
                  ...
                  float[] columnsSize = new float[] {100f,30f};
    ...
    private static void buildTable(Document document, Runsheet r, String barCodeColisDirectoryPath, List<ColisEtat> colisEtat) {
        if(r.getColis().stream().filter(c->colisEtat.contains(c.getEtat())).count()>0) {
            ...
            float[] columnWidths = new float[] {100f,100f,100f, 100f, 100f, 140f,80f, 140f};
            try {
    			...
    			table.setWidths(columnWidths);
    			  Font font10 = FontFactory.getFont(FontFactory.HELVETICA ,12, BaseColor.BLACK);
    		        Stream.of("Code à barre " ,"Expéditeur","Téléphone Expéditeur","Client",  " Téléphone Client" , "Adresse" ,"COD" , "Remarque")
    		                .forEach(headerTitle -> {
    		        ...
    		        Collections.sort(c, new Comparator<Colis>() {
						@Override
						public int compare(Colis o1, Colis o2) {
							return o1.getOrderInRunsheet().compareTo(o2.getOrderInRunsheet());
						}
					});
    		        ...
    		        for(Colis c4 : c)
    		        {
    		            ...
    		            PdfPCell  expCell = new PdfPCell(new Phrase(c4.getFournisseur().getNom_societe() , font10));
    		            ...
    		            /*PdfPCell  expGovCell = new PdfPCell(new Phrase(c4.getFournisseur().getGouvernorat_societe(), font10));
    		            expGovCell.setPaddingLeft(4);
    		            expGovCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
    		            expGovCell.setHorizontalAlignment(Element.ALIGN_CENTER);
    		            expGovCell.setBorderWidth(1);
    		            expGovCell.setBorderColor(BaseColor.BLACK);
    		            table.addCell(expGovCell);*/
    		            
    		            PdfPCell  expTelCell = new PdfPCell(new Phrase(c4.getFournisseur().getTel(), font10));
    		            expTelCell.setPaddingLeft(4);
    		            expTelCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
    		            expTelCell.setHorizontalAlignment(Element.ALIGN_CENTER);
    		            expTelCell.setBorderWidth(1);
    		            expTelCell.setBorderColor(BaseColor.BLACK);
    		            table.addCell(expTelCell);
