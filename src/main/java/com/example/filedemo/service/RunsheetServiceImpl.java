...
public class RunsheetServiceImpl implements RunsheetService {
	...
	public Runsheet addRunsheet(RunsheetPayload runsheetPayload) {
		...
		setOrderInRunsheet(colis, runsheetPayload.getColisBarCodes());
    ...
    private void setOrderInRunsheet(List<Colis>colis, List<String> enteredReferences) {
    	for(Colis c:colis) {
    		c.setOrderInRunsheet(enteredReferences.indexOf(c.getBar_code()));
    	}
    }
	@Override
