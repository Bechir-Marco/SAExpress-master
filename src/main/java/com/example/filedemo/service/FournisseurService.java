...
public class FournisseurService {
	...
	public Fournisseur saveFour(Fournisseur four) {
		...
		if(four.getDate_fin_contrat()==null) {
			try {
				Date dateLivraison=new SimpleDateFormat("yyyy-MM-dd").parse(
						LocalDateTime.now().plusYears(1).toLocalDate().toString());
				dateLivraison.setHours(12);
				four.setDate_fin_contrat(dateLivraison);
